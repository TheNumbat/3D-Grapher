...
Symbolic toSymbolic(const std::vector<op>& EQ) {
	...
	for (unsigned int index = 0; index < EQ.size(); index++) {
		...
		case op_sec:
			get1();
			result = Symbolic(1.0f) / cos(one);
			s.push(result);
			break;
		case op_csc:
			get1();
			result = Symbolic(1.0f) / sin(one);
			s.push(result);
			break;
		case op_cot:
			get1();
			result = Symbolic(1.0f) / tan(one);
			s.push(result);
		case op_asec:
			get1(); 
			result = acos(Symbolic(1.0f) / one);
			s.push(result);
			break;
		case op_acsc:
			get1();
			result = asin(Symbolic(1.0f) / one);
			s.push(result);
			break;
		case op_acot:
			get1();
			result = atan(Symbolic(1.0f) / one);
			s.push(result);
			break;
		case op_neg:
			get1();
		...
		case var: {
			...
			vstr.resize(1);
			vstr[0] = v;
			s.push(Symbolic(wstring_to_utf8(vstr)));
			break;
		}
		case const_e:
			s.push(val_e);
		...
		default:
			...
			s.push(Symbolic((float)atof(num.c_str())));
			break;
		}
	}
	result = s.top();
